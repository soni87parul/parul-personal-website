---
layout: layouts/article.njk
title: "Sometimes the constraint to building a business sits outside the business."
subtitle: ""
excerpt: "In cultivated meat, the immediate business question was simple: when can the product be sold? Answering it meant understanding something larger than one company's licence."
territory: "Policy, Markets & Economic Systems"
tags: []
date: 2026-06-29
coverImage: ""
author: "Parul Soni Manvati"
featured: false
published: true
related_experience: "clearmeat-building-emerging-category"
from_the_field_context: "working on regulatory ecosystem-building for cultivated meat in India"
contact_intent: "policy"
contact_topic: "ecosystem"
takeaway: "When the market infrastructure does not yet exist, strategy expands from building a company to helping create the conditions in which the company can succeed."
seoTitle: "Sometimes the constraint sits outside the business | Parul Soni Manvati"
seoDescription: "Why emerging industries sometimes need company-building and system-building at the same time, drawn from regulatory ecosystem work in cultivated meat."
---
Most business frameworks keep the founder's attention inside the company.

Product.
People.
Customer.
Capital.
Operations.
GTM.

But emerging industries often discover that the most important constraint sits somewhere else.

Regulation.

Standards.

Research infrastructure.

Manufacturing capability.

Public understanding.

Government policy.

An ecosystem of suppliers or specialists that does not yet exist.

At that point, company strategy alone is not enough.

This became clear while working in cultivated meat.

The immediate business question was simple:

When can the product be sold?

But answering it meant understanding something larger than one company's licence.

How should a completely new category be classified?

What standards should apply?

How should manufacturing be treated?

What should the ecosystem learn from markets that moved earlier?

And what infrastructure would startups in the category need if India wanted to build domestic capability rather than simply import it?

<div class="pull-quote"><p>The business problem had become a system problem. That changes the role of strategy.</p></div>

You have to think about stakeholders who are not customers.

Regulators.

Government.

Researchers.

Industry bodies.

Incubators.

Investors.

Other startups.

Sometimes even competitors.

Because the category cannot develop if every company tries to build the surrounding system alone.

This does not mean every startup should become a policy organisation.

It means leaders in emerging industries need to know when the constraint has moved beyond the boundaries of the company.
